...
const PRODUCTION_CARS = [
  ...
  { name: 'Tesla Model 3 Performance',   hp: 450,  z60: 3.1,  qmile: 11.5, r60100:  6.6 },
  { name: 'BMW M3 Competition (G80)',    hp: 503,  z60: 3.4,  qmile: 11.6, r60100:  6.7 },
...
// Uses the Hale/Hollander quarter-mile formula; 0-60 and 60-100 are derived empirically.
function estimateTimes(hp, weightLbs, drivetrainSel, launchFactor = 1.0) {
  ...
  const rollFactor = isAWD ? 0.80 : isFWD ? 1.05 : 0.97;
  const r60100  = Math.max(1.7, wPerHp * rollFactor);

  // launchFactor < 1 = faster times (better tires/launch prep)
  // 60-100 is less dependent on launch, so apply a smaller fraction of it
  return {
    z60:    +(z60    * launchFactor).toFixed(2),
    qmile:  +(qmile  * launchFactor).toFixed(2),
    r60100: +(r60100 * (1 + (launchFactor - 1) * 0.5)).toFixed(1),
  };
}
...
function renderComparison(boostedBHP, weightLbs, drivetrainSel, launchFactor = 1.0) {
  ...
  document.getElementById('your-r60100').textContent = my.r60100 + 's';
  ...
  tbody.innerHTML = sorted.map(car => {
    ...
    const beatRoll  = my.r60100 < car.r60100 - TIE_ROLL;
    const tieZ60    = !beatZ60  && Math.abs(my.z60    - car.z60)    < TIE_Z60;
    const tieQ      = !beatQ    && Math.abs(my.qmile  - car.qmile)  < TIE_Q;
    const tieRoll   = !beatRoll && Math.abs(my.r60100 - car.r60100) < TIE_ROLL;
    ...
    return `<tr class="${rowCls}">
      ...
      ${cell(my.r60100, car.r60100, beatRoll, tieRoll)}
